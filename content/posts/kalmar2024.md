...
Now, we can't work over $F_p$ because $\sqrt{\Delta}$ doesn't exist, so we need to work over $F_{p^2}$, and it is equivalent to $F_p[x]/(x^2 - \Delta)$. After extend the field, we can do the same as Case 1
